...
export class CompareService {
  ...
  onStoreData(data: CompareData) {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session)=>{
      ...
      this.http.post('https://qryjjfu1lg.execute-api.ca-central-1.amazonaws.com/dev/compare-yourself', data, {
  ...
  onRetrieveData(all = true) {
    ...
    this.authService.getAuthenticatedUser().getSession((err, session)=>{
      ...
      let urlParam = 'all';
      if(!all){urlParam = 'single';}
      this.http.get(' https://qryjjfu1lg.execute-api.ca-central-1.amazonaws.com/dev/compare-yourself/' + urlParam + queryParam, {
        headers: new Headers({'Authorization': session.getIdToken().getJwtToken()})
      })
        .map(
